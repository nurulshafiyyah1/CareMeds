...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($appointment_id) || !$patient_id || empty($patient_name) || !$patient_age || 
        empty($patient_gender) || empty($hospital) || empty($doctor) || empty($date_time) || 
        empty($type) || empty($status) || empty($symptoms) || empty($nurse_id)) {
    ...
    header("Location: appointments.php?success_edit=1");
    exit;
} else {
